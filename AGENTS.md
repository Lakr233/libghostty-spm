...
5. **Text positioning** → `TerminalTextPosition` / `TerminalTextRange` (UITextPosition/UITextRange subclasses) provide minimal cursor geometry. On iOS the document starts with `TerminalTextInputHandler.documentAnchorLength` (1) empty position ahead of the marked text, and every position UIKit sees is in those coordinates: UIKit stops a held software-keyboard Delete whenever `-[UIResponder _selectionAtDocumentStart]` (selection start == `beginningOfDocument`) is true, and with an empty document at a prompt that was always, so a held Delete sent one backspace. The anchor reads as no text; geometry subtracts it. `caretRect`/`firstRect` use `surface.imePoint()` for IME candidate window placement.
